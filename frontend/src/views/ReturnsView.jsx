...
import { Undo2, Check, AlertTriangle, Download, Plus, Search, Edit2, Trash2, Filter } from 'lucide-react';
...
export default function ReturnsView() {
  ...
  const handleDamage = async (id) => { if (!window.confirm('Write off this return as damaged / loss?')) return; try { await api.damageReturn(id); loadReturns(); } catch (err) { alert(err.message); } };
  const handleBulkRestock = async () => { try { const res = await api.bulkRestockReturns(); alert(res.message); loadReturns(); } catch (err) { alert(err.message); } };
  const handleDelete = async (id) => { if (!window.confirm('Delete customer return entry?')) return; try { await api.deleteReturn(id); loadReturns(); } catch (err) { alert(err.message); } };
  ...
  const searchFiltered = returns.filter((r) => {
    const matchSearch = r.style_no.toLowerCase().includes(search.toLowerCase()) || (r.primary_reason && r.primary_reason.toLowerCase().includes(search.toLowerCase())) || (r.reverse_awb && r.reverse_awb.toLowerCase().includes(search.toLowerCase()));
    const matchStatus = statusFilter === 'ALL' || (statusFilter === 'Received' ? (r.status === 'Received' || r.status === 'Intake') : r.status.toLowerCase() === statusFilter.toLowerCase());
  ...
  const getStatusPill = (status) => {
    switch (status) {
      case 'In Transit': return <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30">In Transit</span>;
    ...
    <div className="space-y-4">
      ...
      <div className="glass-panel p-4 flex flex-col md:flex-row items-center justify-between gap-3" data-tour="returns-hud">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
          {holdingCount > 0 && (<button onClick={handleBulkRestock} className="btn btn-success text-xs px-3.5 h-9" title="Restock all received return units back into inventory" data-tour="returns-bulk-restock"><Check className="w-3.5 h-3.5" /><span>Bulk Restock Dock ({holdingCount})</span></button>)}
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-900/90 text-slate-400 font-semibold border-b border-slate-800">
              <tr>
                ...
                <SortableHeader label="Style SKU" columnKey="style_no" sortable filterable {...sharedHeaderProps} activeFilterValues={columnFilters['style_no']} extraProps={{ 'data-tour': 'returns-col-sku' }} />
                ...
                <SortableHeader label="Status" columnKey="status" sortable filterable align="center" {...sharedHeaderProps} activeFilterValues={columnFilters['status']} extraProps={{ 'data-tour': 'returns-col-status' }} />
              ...
              {loading ? (<tr><td colSpan="14" className="py-8 text-center text-slate-400">Loading customer returns...</td></tr>
                ...
                filtered.map((r, idx) => (
                  <tr key={r.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center font-mono text-[11px] text-slate-400 whitespace-nowrap">{r.received_date || '—'}</td>
                    ...
                    <td className="py-3 px-4 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setQcModalReturn(r)} className="p-1 text-slate-400 hover:text-white" aria-label="Inspect QC"><Edit2 className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDelete(r.id)} className="p-1 text-slate-400 hover:text-rose-400" aria-label="Delete return"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))
          ...
          <div className="glass-panel w-full max-w-lg p-6 border border-slate-700 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><label htmlFor="return-date" className="text-xs text-slate-300 block mb-1">Return Initiated Date *</label><input id="return-date" name="return_date" type="date" value={date} onChange={(e) => setDate(e.target.value)} required className="input-field" /></div>
                <div><label htmlFor="return-qty" className="text-xs text-slate-300 block mb-1">Quantity *</label><input id="return-qty" name="return_qty" type="number" min="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} required className="input-field" /></div>
              </div>
              ...
              <div><label htmlFor="return-reason" className="text-xs text-slate-300 block mb-1">Primary Return Reason *</label>
                <select id="return-reason" name="return_reason" value={primaryReason} onChange={(e) => setPrimaryReason(e.target.value)} className="input-field">
                  <option value="Size Too Small / Fit Issue">Size Too Small / Fit Issue</option><option value="Size Too Large / Fit Issue">Size Too Large / Fit Issue</option><option value="Fabric Quality Issue">Fabric Quality Issue</option><option value="Wrong Color / Style Sent">Wrong Color / Style Sent</option><option value="Received Defective Product">Received Defective Product</option><option value="Did not like the product">Did not like the product</option><option value="Don't need the product anymore">Don't need the product anymore</option><option value="Other">Other</option>
                </select>
              </div>
      ...
      {qcModalReturn && (<CustomerReturnEditModal ret={qcModalReturn} isOpen={true} onClose={() => setQcModalReturn(null)} onSuccess={loadReturns} />)}
    </div>
  );
}
